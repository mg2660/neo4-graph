...
confirmationShown: boolean = false;
  ...
  constructor(private http: HttpClientprivate http: HttpClient) {}
...
• Event Dates: ${startDate} - ${endDate}`,
  ...
  getBestRoutesFromTopology(): void {
    this.addBotMessage("⏳ Getting the best route, please wait...");

    this.http.get<any>('http://localhost:3000/api/routes').subscribe({
      next: (response) => {
        const { bestPath, alternativePaths } = response;

        let message = `✅ Best Route: ${bestPath.join(' ➡️ ')}
`;

        if (alternativePaths && alternativePaths.length > 0) {
          message += `🛣️ Alternatives:
`;
          alternativePaths.forEach((path: string[], idx: number) => {
            message += `• ${path.join(' ➡️ ')}
`;
          });
        }
      ...
      error: (err) => {
        ...
        this.addBotMessage("⚠️ Couldn't fetch route data. Please ensure the Topology server is running.");
      }
    });
  }
